...
Chapter 6, determine confidence interval
"""
...
def solve_aggregation(product,reactions,n,L,R,nonzeros,existing, timeaxis):
    if (R-L)<0.0001:
        return (R+L)/2
    sum1 = calculate_sum1(reactions,n)
    sum2 = calculate_sum2(product,reactions,n,(L+R)/2,nonzeros, timeaxis)
    value = calculate_sum4(product,reactions,n,(L+R)/2,sum1,sum2,existing, timeaxis)
    print(L, R, "value of middle point:" , value)
    if value<0: #het is een monotoon dalende functie
        return solve_aggregation(product,reactions,n,L,(L+R)/2,nonzeros,existing, timeaxis)
    return solve_aggregation(product,reactions,n,(L+R)/2,R, nonzeros,existing, timeaxis)
...
def calculate_sum2(product,reactions,n,alfa,nonzeros,timeaxis):
    sum_total= 0
        ...
        for i in nonzeros[k]:
            for j in nonzeros[k]:
                if i<=j: #prevent doubles
                    if i==j:
                        sum_total =sum_total + ((i+1)*(1+j))**alfa * product[k][i]*product[k][j]/2 * (timeaxis[k+1]-timeaxis[k])         
                    else:  
                        sum_total =sum_total + ((i+1)*(1+j))**alfa * product[k][i]*product[k][j]   * (timeaxis[k+1]-timeaxis[k])            
    return sum_total

def calculate_sum3(product,reactions,n,alfa,i,j,sum1,sum2,timeaxis):
    sum_total = 0
    for k in range(len(reactions)):
        if ((product[k][i]!=0) & (product[k][j]!=0)):
            if i==j:
                sum_total = sum_total + find_reaction(k, i, j, reactions) - sum1/sum2 * ((i+1)*(1+j))**alfa * product[k][i]*product[k][j]/2 * (timeaxis[k+1]-timeaxis[k])
            else:
                sum_total = sum_total + find_reaction(k, i, j, reactions) - sum1/sum2 * ((i+1)*(1+j))**alfa * product[k][i]*product[k][j] * (timeaxis[k+1]-timeaxis[k])
    return sum_total   

def calculate_sum4(product,reactions,n,alfa,sum1,sum2,existing,timeaxis):
    sum_total= 0
    for i in existing:
        for j in existing:
            if i<=j:
                
                sum_total = sum_total + np.log((i+1)*(1+j))*calculate_sum3(product,reactions,n,alfa,i,j,sum1,sum2,timeaxis)
    return sum_total     

def determine_C(product,reactions,n,alfa,tau, nonzeros,existing,timeaxis):
    sum1 = calculate_sum1(reactions,n)
    sum2 = calculate_sum2(product,reactions,n,alfa,nonzeros,timeaxis)
    return (sum1/sum2)
